      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="p-5">
          ...
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold text-gray-900 dark:text-white">ブックマーク編集</h2>
            <button
              onClick={onClose}
          ...
          <div className="mb-3">
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500 truncate">{bookmark.url}</p>
          </div>

          {/* タイトル */}
          <div className="mb-3">
            <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">タイトル</label>
            <input
              ref={titleRef}
              type="text"
              value={title}
          ...
          {/* タグ */}
          <div className="mb-4">
            <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">タグ</label>
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="タグを追加..."
                className="flex-1 px-3 py-1.5 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm text-gray-900 dark:text-white
                           focus:outline-none focus:ring-2 focus:ring-indigo-400"
              />
              <button
            ...
            {suggestedTags.length > 0 && (
              <div className="mt-2">
                <p className="text-xs text-gray-400 dark:text-gray-500 mb-1">既存のタグ：</p>
                  ...
                  {suggestedTags.map((tag) => (
                    <button
                      ...
                      className="px-2 py-0.5 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 text-xs rounded-full
                                 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors"
                    >
                      +{tag}
                    </button>
                  ))}
